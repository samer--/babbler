...
s        ---> s(_).
s(Tense) ---> #tense(Tense), island(np(P,N,G,nominative,A)), vp(P,N,G,A,_,tensed(Tense)).
...
verb_adverb(past_part,tensed).
 ...
 maprule( vp(_,_,_,_,_,VF), vp(_,_,_,_,_,VF)).

% General rule for ordinary verbs with any arguments
8 # vp(Pers,Num,Gen,Agency,[ordinary],VForm) --->
...
%%	({VForm=tensed(_)}->maybe([not])),
	vp(Pers,Num,Gen,Agency,TD,gerund).
...
% negation of verb
vp(Pers,Num,Gen,Ag,[not(V)|TD],VForm) --->
	[not],
	{when(nonvar(VForm),(VForm\=tensed(_),VForm\=imperative))},
	vp(Pers,Num,Gen,Ag,[V|TD],VForm),
...
1 # np(P,M,G,F,A) +--> np(P,M,G,F,A), cm, vp(P,M,G,A,_,gerund), cm. % GAP?
...
2 # object(P,N,G,A,P1,N1,G1,A1) ---> fills(object(P,N,G,A,P1,N1,G1,A1),[]).
